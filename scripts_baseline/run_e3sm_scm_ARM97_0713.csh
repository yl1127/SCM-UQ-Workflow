...
#######  BEGIN USER DEFINED SETTINGS
  ...
  setenv casename e3sm_scm_ARM97_implicit_stress
  ...
  # Code tag name. Not used when E3SM_CODE_DIR points directly at an E3SM checkout.
  setenv code_tag E3SM_codetag

  # Optional Mac-only ML4ESM embedded CNN trigger support.
  ...
  setenv machine Mac
  if ($use_ml4esm == true && $machine != Mac) then
    echo "ERROR: ENABLE_ML4ESM is currently supported only for machine Mac."
    exit 1
  endif

  # Want to submit run to the queue?
  #   Setting to false runs locally through case.submit --no-batch.
...
# End Case specific stuff here
  ...
  if ($use_ml4esm == true && $atm_mod != eam) then
    echo "ERROR: ENABLE_ML4ESM requires an EAM-based E3SM configuration."
    exit 1
  endif
...
# Use implicit surface stress coupling for this sensitivity experiment.
...
cat <<EOF >> user_nl_${atm_mod}
 zmconv_trigdcape_ull = .true.
 zmconv_ke = 2.5E-6
 nucleate_ice_subgrid = 1.35D0
 p3_wbf_coeff = 1.0
 p3_max_mean_rain_size = 0.005D0
 p3_embryonic_rain_size = 0.000025D0
 p3_autocon_coeff = 30500.0D0
 p3_qc_autocon_expon = 3.19D0
 p3_nc_autocon_expon = -1.10D0
 p3_accret_coeff = 117.25D0
 p3_qc_accret_expon = 1.15D0
...
# Modify the latitude and longitude for the particular case
  ...
  if ($use_ml4esm == true) then
    ...
    mkdir -p SourceMods/src.${atm_mod}
    cp -R ${ML4ESM_DIR}/SourceMods/src.eam/. SourceMods/src.${atm_mod}/
...
EOF
    ...
    setenv ML4ESM_MODEL_DIR ${case_scripts_dir}/SourceMods/src.${atm_mod}/pytorch_1124
    setenv ML4ESM_PYTHONHOME $py_prefix
    setenv ML4ESM_PYTHONPATH ${case_scripts_dir}/SourceMods/src.${atm_mod}
    if ($py_libdir != "") then
      setenv DYLD_LIBRARY_PATH ${py_libdir}
    endif

cat <<EOF >> .env_mach_specific.sh
export ML4ESM_MODEL_DIR="${case_scripts_dir}/SourceMods/src.${atm_mod}/pytorch_1124"
export ML4ESM_PYTHONHOME="$py_prefix"
export ML4ESM_PYTHONPATH="${case_scripts_dir}/SourceMods/src.${atm_mod}"
...
# Reuse a compatible executable when TEMPLATE_EXE_MAC (or TEMPLATE_EXE) is
# provided. Otherwise retain the original behavior and build a fresh executable.
  set template_exe = ""
  ...
  if ("$template_exe" == "") then
    ...
    if (! -x "$template_exe") then
      echo "ERROR: template executable is not executable: $template_exe"
      exit 1
    endif
